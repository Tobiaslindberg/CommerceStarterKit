...
(function ($, Oxx) {
	...
	Oxx.HotSpotsEditor.HotSpot.prototype = {
		...
		init: function () {
			...
			this._$hotSpot.on('mousedown', $.proxy(this._onHotSpotMouseDown, this));
			this._$imageMapArea.on('mousedown', $.proxy(this._onImageMapAreaMouseDown, this));
		...
		_enableImageMapAreaResizing: function () {
			this._$imageMapArea.resizable({
				...
				stop: $.proxy(this._onImageMapAreaResize, this)
			}).draggable({
				revert: false,
				containment: this._$container,
				scroll: false,
				stop: $.proxy(this._onImageMapAreaDragged, this)
			});
		},
		...
		_enableHotSpotDragging: function() {
			...
			this._$hotSpot.draggable({
				...
				scroll: false,
				stop: $.proxy(this._onHotSpotDragged, this)
			});
