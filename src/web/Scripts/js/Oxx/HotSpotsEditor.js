...
(function ($, Oxx) {
	...
	Oxx.HotSpotsEditor = function (container, options) {
		...
		if(this._settings.options.hotSpotImageWidth === 0) {
			var me = this;
			setTimeout(function() {
				me._$newHotSpotButton.getBackgroundSize($.proxy(me._callbackHotSpotImageSizeReturned, me));
			}, 100);
		} else {
			this.init();
		}
	...
	Oxx.HotSpotsEditor.prototype = {
		...
		init: function () {
			...
			this._$imageContainer.droppable({
				accept: '.hotspot.new',
				tolerance: 'pointer',
				drop: $.proxy(this._onNewHotSpotDropped, this)
			});
			...
			this._$trash.droppable({
				accept: '.hotspot',
				tolerance: 'pointer',
				drop: $.proxy(this._onHotSpotDroppedInTrash, this)
			});
		...
		/**
		 * Get the value from the hidden box
		 *
		 * @returns {string}
		 */
		getValue: function() {
		...
		_addHotSpotEvents: function(hotSpot) {
			$(hotSpot).on('change', $.proxy(this._onHotSpotChanged, this));
			$(hotSpot).on('select', $.proxy(this._onHotSpotSelected, this));
		},
		...
		_saveChanges: function() {
			var originalValue = this._$value.val(),
				hotSpots = [];
